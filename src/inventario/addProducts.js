...
// variables
  const { nombre, unidades, costo, precio, idUsuario, laboratorio } = req.body;
  ...
  conexion.query("SELECT * FROM productos WHERE nombre =?", [nombre], (err, result) => {
    if (err) {
      ...
      if (result.length <= 0) {
        ...
        conexion.query("INSERT INTO productos SET ? ", [{ nombre, unidades, costo, precio, id_usuario: idUsuario, laboratorio }], async (err, result) => {
          if (err) {
